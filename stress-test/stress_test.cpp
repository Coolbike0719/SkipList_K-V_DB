...
SkipList<int, std::string> skipList(18, std::chrono::seconds(1), 100);
// 插入数据线程
...
// 定时清理线程
void cleanup_thread(SkipList<int, std::string>* list) {
    while (true) {
        std::this_thread::sleep_for(std::chrono::seconds(2)); // 每2秒清理一次
...
int main() {
    // 启动清理线程
    // std::thread cleanup_thread_instance(cleanup_thread, &skipList);
